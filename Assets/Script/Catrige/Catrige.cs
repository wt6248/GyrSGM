...
public class Catrige : MonoBehaviour
{
    ...
    public void FireCatrige(Vector3 lineOfFire, Entity.EntityType attackableEntityType, Vector3 generationPosition)
    {
        ...
        for (int i = 0; i < _bulletPelletCount; i++)
        {
            ...
            Debug.Log((i+1).ToString() + ": "+ spreadVector);
            //총알을 생성함.
            GenerateBullet(spreadVector, attackableEntityType, generationPosition);
        }
    }

    //총알을 생성하는 함수이다.
    protected virtual void GenerateBullet(Vector3 bulletDirection, Entity.EntityType attackableEntityType, Vector3 generationPosition)
    {
        ...
        instance.GetComponent<BulletScript>().Activate(_bulletSpeed, bulletDirection, _bulletDamage, _knockbackDistance, _duration, _maxPenetration, attackableEntityType);
    }
